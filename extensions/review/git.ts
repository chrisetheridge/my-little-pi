...
function getPorcelainEntries(cwd: string, runner = defaultCommandRunner): PorcelainEntry[] {
  const parts = git(cwd, ["status", "--porcelain=v1", "-z", "--untracked-files=all"], runner)
    .stdout.split("\0")
    .filter(Boolean);
...
export function buildCommitReviewTarget(cwd: string, commitRef: string): ReviewTarget {
  ...
  const files = isRootCommit
    ? lines(
        git(cwd, ["diff-tree", "--root", "--no-commit-id", "--name-only", "-r", resolvedCommit])
          .stdout,
      )
    : isMergeCommit
      ? lines(git(cwd, ["diff", "--name-only", `${firstParent}..${resolvedCommit}`]).stdout)
      : lines(
          git(cwd, ["diff-tree", "--no-commit-id", "--name-only", "-r", resolvedCommit]).stdout,
        );
...
export function buildPullRequestReviewTarget(
  ...
  if (!checkout.ok) {
    throw new Error(
      checkout.stderr.trim() ||
        ...
        `Could not checkout pull request ${prUrl}`,
    );
  }
  ...
  try {
    ...
    if (!changedFilesResult.ok) {
      throw new Error(
        changedFilesResult.stderr.trim() ||
          changedFilesResult.stdout.trim() ||
          "Could not read PR files",
      );
    }
    diffResult = runCommand(cwd, "gh", ["pr", "diff", prUrl, "--patch"], runner);
    if (!diffResult.ok) {
      throw new Error(
        diffResult.stderr.trim() || diffResult.stdout.trim() || "Could not read PR diff",
      );
    }
  } catch (error) {
...
export function restoreOriginalRef(
  cwd: string,
  originalRef: string,
  runner = defaultCommandRunner,
): boolean {
