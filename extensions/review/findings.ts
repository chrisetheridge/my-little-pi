...
function stableFindingId(
  finding: Omit<ReviewFinding, "id" | "severity" | "suggestedFix" | "status">,
): string {
...
export function loadSourceExcerpt(
  ...
  try {
    if (!existsSync(resolved.path)) {
      return {
        available: false,
        message: "Source file is unavailable: file not found.",
        lines: [],
      };
    }
    if (!statSync(resolved.path).isFile()) {
      return {
        available: false,
        message: "Source file is unavailable: not a regular file.",
        lines: [],
      };
    }
    ...
    const endLine =
      location.endLine === undefined ? startLine : requireLine(location.endLine, "endLine");
    if (endLine < startLine) {
      return {
        available: false,
        message: "Source location is unavailable: endLine is before startLine.",
        lines: [],
      };
    }

    const sourceLines = readFileSync(resolved.path, "utf-8")
      .split(/\n/)
    ...
    if (startLine > sourceLines.length) {
      return {
        available: false,
        message: "Source location is unavailable: startLine is out of range.",
        lines: [],
      };
    }
