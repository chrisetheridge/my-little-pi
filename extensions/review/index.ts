...
import {
  buildFindingsFormatterPrompt,
  buildReviewFixPrompt,
  buildReviewPrompt,
  formatExcerptForPrompt,
} from "./prompt.ts";
...
export async function formatFindingsWithSelectedModel(
  ...
  if (response.stopReason !== "stop") {
    throw new Error(
      response.errorMessage ?? `formatter model stopped with ${response.stopReason}.`,
    );
  }
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            reviewCtx.ui.notify(
              `Could not recover review findings: ${(formatterError as Error).message}`,
              "error",
            );
            return;
          }
