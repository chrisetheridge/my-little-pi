...
class FixNotePromptDialog implements Component, Focusable {
  ...
  constructor(
    ...
    this.panel.addChild(
      new Text(
        this.theme.fg("text", "Leave a note for this finding, then press Enter to save."),
        0,
        0,
      ),
    );
  ...
  render(width: number): string[] {
    ...
    const lines = [this.theme.fg("borderAccent", `┌${"─".repeat(Math.max(0, borderWidth - 2))}┐`)];
...
export class FindingsDialog {
  ...
  constructor(
    ...
    private readonly promptForFixNote: (
      title: string,
      defaultValue: string,
    ...
    this.state =
      state.currentIndex < 0 || state.currentIndex > maxIndex
        ? updateReviewIndex(state, state.currentIndex)
        : state;
  }
  ...
  private buildView(width: number): string[] {
    ...
    const visibleBodyLines = bodyLines.slice(
      this.scrollOffset,
      this.scrollOffset + FINDINGS_VIEWPORT_LINES,
    );
    ...
    lines.push(
      this.boxLine(
        width,
        this.theme.fg("dim", truncateToWidth(this.buildFooterHint(maxScroll > 0), innerWidth)),
      ),
    );
    lines.push(this.borderLine(width, "└", "─", "┘"));
    return lines;
  ...
  private buildBodyContent(contentWidth: number): Container {
    ...
    if (!excerpt.available) {
      content.addChild(
        new Text(this.theme.fg("dim", excerpt.message ?? "Source unavailable."), 0, 0),
      );
    } else {
      for (const excerptLine of excerpt.lines) {
        ...
        content.addChild(
          new Text(
            excerptLine.selected
              ? this.theme.fg("accent", lineText)
              : this.theme.fg("dim", lineText),
            0,
            0,
          ),
        );
  ...
  private buildFooterHint(canScroll: boolean): string {
    ...
    const scrollHint = canScroll
      ? "  Up/Down or j/k: scroll  PgUp/PgDn: page  Home/End: top/bottom"
      : "";
...
export async function showFindings(
  ...
  const result = await ctx.ui.custom<FindingsDialogResult>(
      ...
      new FindingsDialog(
        ...
        (stateToClose) => {
          ...
          return ctx.ui.confirm(
            "Exit review?",
            `${openCount} finding${openCount === 1 ? "" : "s"} still retained.`,
          );
        },
        (title, defaultValue) =>
    ...
    {
      overlay: true,
      overlayOptions: { anchor: "center", width: "72%", minWidth: 72, maxHeight: "85%", margin: 2 },
    },
  );
