...
export function loadLandingWorkflowConfig(cwd: string, home = homedir()): ConfigLoadResult {
  ...
  const source = existsSync(paths.project) ? paths.project : existsSync(paths.global) ? paths.global : undefined;

  if (!source) {
    return {
      ok: false,
      expectedProjectPath: paths.project,
      error: `No landing workflow config found. Create ${paths.project} to enable /land.`,
    };
  }
  ...
  if (!validation.ok) {
    return { ok: false, source, expectedProjectPath: paths.project, error: validation.error };
  }
  return { ok: true, source, config: validation.config };
}

export function validateLandingWorkflowConfig(value: unknown):
  | { ok: true; config: LandingWorkflowConfig }
  | { ok: false; error: string } {
  if (!isRecord(value)) return { ok: false, error: "Config must be a JSON object." };
  if (!Array.isArray(value.steps)) return { ok: false, error: "Config must include a steps array." };
  ...
  for (let index = 0; index < value.steps.length; index += 1) {
    ...
    if (!isNonEmptyString(raw.label)) {
      return { ok: false, error: `Step ${index + 1} must have a non-empty label.` };
    }
    if (raw.type === "shell") {
      if (!isNonEmptyString(raw.command)) {
        return { ok: false, error: `Shell step ${index + 1} must have a non-empty command.` };
      }
      steps.push({ type: "shell", label: raw.label.trim(), command: raw.command.trim() });
      continue;
    }
    if (raw.type === "commit") {
      if (!isNonEmptyString(raw.model)) {
        return { ok: false, error: `Commit step ${index + 1} must have a non-empty model.` };
      }
      steps.push({ type: "commit", label: raw.label.trim(), model: raw.model.trim() });
      continue;
    }
    return { ok: false, error: `Step ${index + 1} has unknown type ${JSON.stringify(raw.type)}.` };
  }
