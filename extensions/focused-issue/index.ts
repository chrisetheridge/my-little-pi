...
function updateWidget(
  ...
  controller: FocusedIssueController,
  scrollState: FocusedIssueScrollState,
): void {
  if (!ctx?.hasUI) return;
...
function notifyError(ctx: ExtensionContext | undefined, state: FocusedIssueState): void {
  ...
  ctx.ui.notify(
    `Error focusing issue: ${state.error.message}`,
    state.error.retryable ? "warning" : "error",
  );
}
...
function buildBeforeAgentStartResult(
  controller: FocusedIssueController,
): BeforeAgentStartEventResult | undefined {
  const state = controller.getState();
  if (shouldInjectReady(state) && state.issue) {
    const message = buildFocusedIssueMessage(
      formatFocusedIssueContext(state.issue),
  ...
  if (shouldInjectPending(state)) {
    const message = buildFocusedIssueMessage(
      formatPendingIssueContext(state),
      controller.toSnapshot(),
    );
...
export function createFocusedIssueExtension(
  ...
  return function focusedIssueExtension(pi: ExtensionAPI): void {
    ...
    pi.registerCommand("focus-issue", {
      ...
      getArgumentCompletions: (prefix) => {
        ...
        return COMMANDS.filter((command) => command.startsWith(normalized)).map((command) => ({
          label: command,
          value: command,
        }));
      },
      handler: async (args, ctx) => {
        ...
        if (command === "refresh" || command === "inject") {
          ...
          ctx.ui.notify(
            command === "inject"
              ? "Focused issue will be reinjected after refresh"
              : "Focused issue refresh started",
            "info",
          );
          return;
        }
