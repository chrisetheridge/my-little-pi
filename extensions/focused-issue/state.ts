...
export function restoreFocusedIssueSnapshot(entries: unknown[]): FocusedIssueSnapshot | undefined {
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    const entry = entries[index] as
      | { type?: unknown; customType?: unknown; data?: unknown }
      | undefined;
...
export class FocusedIssueController {
  ...
  restore(snapshot: FocusedIssueSnapshot | undefined): void {
    ...
    this.state = {
      ...snapshot,
      status:
        snapshot.status === "loading" || snapshot.status === "stale" ? "stale" : snapshot.status,
  ...
  refresh(options: { reinject?: boolean } = {}): FocusedIssueState {
    ...
    const provider = this.state.providerId
      ? this.providers.find((candidate) => candidate.id === this.state.providerId)
      : undefined;
    if (!provider) {
  ...
  private startFetch(
    ...
    reference: string,
    token: number,
    previousState: FocusedIssueState,
  ): void {
