  ...
  if (state.issue) {
    ...
    if (state.issue.pullRequests.length) {
      ...
      for (const pullRequest of state.issue.pullRequests.slice(0, 3)) {
        ...
        lines.push(
          `| ${title} | ${escapeTableCell(pullRequest.status ?? "")} | ${escapeTableCell(pullRequest.repository ?? "")} |`,
        );
      }
    }
  } else {
...
export function makeFocusedIssueWidgetFactory(
  ...
  return (_tui: unknown, theme: Theme): Component =>
    new FocusedIssueWidget(getState, theme, now, getScrollOffset, setScrollOffset);
}
...
function renderBorderedMarkdown(
  ...
  const { lines: visible, scrollOffset: effectiveScrollOffset } = sliceScrollableLines(
    rendered,
    scrollOffset,
  );
  ...
  const body = visible.map(
    (line) => `${borderColor("│")} ${padLine(line, innerWidth)} ${borderColor("│")}`,
  );
