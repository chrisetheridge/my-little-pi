...
function normalizePullRequests(attachments: unknown): IssuePullRequest[] {
  return nodesFrom(attachments)
    ...
    .map((attachment) => {
      const title =
        stringValue(attachment.title) ?? stringValue(attachment.subtitle) ?? "Pull request";
    ...
    .filter(
      (pullRequest) =>
        pullRequest.url.includes("github.com") || pullRequest.url.includes("gitlab.com"),
    );
}
  ...
  return {
    ...
    async fetchIssue(reference, signal): Promise<IssueProviderResult> {
      ...
      if (!parsed) {
        return {
          ok: false,
          error: providerError("invalid", "Could not parse Linear issue reference.", false),
        };
      }
      if (!apiKey) {
        return {
      ...
      try {
        ...
        if (signal.aborted) {
          return {
            ok: false,
            error: providerError("cancelled", "Issue fetch was cancelled.", true),
          };
        }
        const message =
          error instanceof Error ? error.message : "Network error while fetching Linear issue.";
      ...
      if (response.status === 401 || response.status === 403) {
        return {
          ok: false,
          error: providerError("auth", "Linear rejected the configured API key.", true),
        };
      }
      if (!response.ok) {
        return {
      ...
      if (!isRecord(body)) {
        return {
          ok: false,
          error: providerError("unknown", "Linear returned an invalid response.", true),
        };
      }
      if (Array.isArray(body.errors) && body.errors.length > 0) {
        return {
          ...
          error: providerError(
            "unknown",
            "Linear returned GraphQL errors while fetching the issue.",
            true,
          ),
        };
      }
      ...
      if (!issue) {
        return {
          ...
          error: providerError(
            "not_found",
            `Linear issue "${parsed.display}" was not found.`,
            false,
          ),
        };
      }
